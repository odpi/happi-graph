    ...
    d3.select(this)
      .call(function (selection: any) {
        if (d.properties) {
          let labelHeight = 80;
          let iconHeight = 80;
          const PROPERTY_MAX_LENGTH = 20;

          for (const p of d.properties) {
            const propertyGroup = selection.append('g').classed('property-group', true);
            ...
            property
             .on('mouseover', function (d: any) {
                ...
                if(textLength > PROPERTY_MAX_LENGTH) {
                  const dataLabelHeight = parseInt(currentNode.attr('data-label-height'));
                  const value = currentNode.attr('data-value');

                  const fullPropertyBackground = d3.select(d.currentTarget.parentNode)
                                      .append('rect')
                                      .classed('full-property-background', true)
                                      .attr('transform', `translate(70, ${dataLabelHeight - 40})`);

                  const fullProperty: any = d3.select(d.currentTarget.parentNode)
                                        .append('text')
                                        .classed('full-property', true)
                                        .attr('transform', `translate(78, ${dataLabelHeight - 32})`)
                                        .text(() => value);

                  const localBBox = fullProperty.node().getBBox();
...
const addNodes = (nodes: any, nodesGroup: any, graphDirection: string, onNodeClick?: any) => {
  ...
  const nodeGroup =
    _nodesGroup
      .append('g')
      .classed('node-group', false)
      .attr('id', (d: any) => d.id)
      .on('click', (d: any) => { onNodeClick ? onNodeClick(d.target.__data__.tex) : console.log('ON_NODE_CLICK_NOT_IMPLEMENTED'); })
...
const centerToCoordinates = function (data: any, scaledBy: any, svg: any, zoom: any, callback: any) {
...
const initCenterGraph = (allGroup: any, svg: any, zoom: any, callback: any) => {
  centerGraph(allGroup, svg, zoom, callback);
}

const centerGraph = (allGroup: any, svg: any, zoom: any, callback?: any) => {
  const graphBBox = allGroup.node().getBBox();
  ...
  const scaledBy = Math.min(
    (svgWidth - 100) / graphBBox.width,
    (svgHeight - 100) / graphBBox.height,
    1
  );

  centerToCoordinates(data, scaledBy, svg, zoom, callback || (() => { console.log('CENTERED')}));
}
...
export const getLinkCoordinates = (nodeA: any, nodeB: any, graphDirection: string) => {
  const _relativeTo: any = relativeTo(nodeA, nodeB, graphDirection);


  const from: any = getNodeAnchorPoint(nodeA, _relativeTo.a);
  const to: any = getNodeAnchorPoint(nodeB, _relativeTo.b);
